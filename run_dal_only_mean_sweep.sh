...
# No INIT_CHECKPOINT — start from foundation weights only
# so DAL contribution is not confounded by HA pre-training

DAL_LAMBDA_VALUES=(0.0 0.1 0.5 1.0 2.0 5.0)

echo "Launching DAL-only sweep — GAP model — HA completely off"

for DAL_LAMBDA in "${DAL_LAMBDA_VALUES[@]}"; do

    LAMBDA_TAG=$(echo "$DAL_LAMBDA" | sed 's/\./p/')
    RUN_NAME="sweep_dalonly__gap__ha0__dall${LAMBDA_TAG}__ep${EPOCHS}__lr1e5"
...
echo "All GAP DAL-only sweep jobs submitted."