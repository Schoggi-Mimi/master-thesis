...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train ConvNeXt baseline on HAM -> BCN.")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if save_every_epoch:
            ...
            save_checkpoint(ep_path, model, optimizer, scheduler, epoch, val_metrics["balanced_accuracy"], stage_name,
                            class_to_idx, idx_to_class, model_name, image_size)
...
def main() -> None:
    ...
    print("=" * 80)
    base_model = build_model(args.model_name, num_classes=len(PRIMARY_CLASSES), pretrained=True).to(device)
    base_model, base_out = fit_stage(
        ...
        stage_name="base_ham",
        checkpoint_path=ckpt_dir / "base_best.pt",
        metrics_json_path=metrics_dir / "base_val_metrics.json",
        device=device,
    ...
    print("=" * 80)
    ft_model = build_model(args.model_name, num_classes=len(PRIMARY_CLASSES), pretrained=False).to(device)
    load_checkpoint_into_model(ckpt_dir / "base_best.pt", ft_model, map_location=device)
    ft_model, ft_out = fit_stage(
        ...
        checkpoint_path=ckpt_dir / "bcn_ft_best.pt",
        metrics_json_path=metrics_dir / "bcn_val_metrics.json",
        device=device,
        class_names=PRIMARY_CLASSES,
        class_to_idx=class_to_idx,
        idx_to_class=idx_to_class,
    ...
    print("=" * 80)
    best_ft_model = build_model(args.model_name, num_classes=len(PRIMARY_CLASSES), pretrained=False).to(device)
    load_checkpoint_into_model(ckpt_dir / "bcn_ft_best.pt", best_ft_model, map_location=device)
    ...
    save_json(test_metrics, metrics_dir / "test_bcn_metrics.json")

    test_cm = np.array(test_metrics["confusion_matrix"])
    plot_confusion_matrix(test_cm, PRIMARY_CLASSES, "BCN Test Confusion Matrix", fig_dir / "test_bcn_confusion_matrix.png")

    test_pred_df = predict_with_metadata(best_ft_model, test_bcn_loader, device, idx_to_class)
    test_pred_df.to_csv(pred_dir / "test_bcn_predictions.csv", index=False)
    ...
    confusions.to_csv(metrics_dir / "test_bcn_top_confusions.csv", index=False)
    ...
    save_json(summary, metrics_dir / "baseline_experiment_summary.json")
    print("=" * 80)
    print("DONE")
    print("=" * 80)
