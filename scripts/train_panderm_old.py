...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--split-dir", type=str, default="data/processed/splits")
    parser.add_argument("--output-dir", type=str, default="outputs/panderm_base")
...
def build_panderm_base_model(num_classes: int) -> nn.Module:
    # PanDerm Base in the README is ViT-B/16.
    # We use the matching timm architecture and then load the PanDerm checkpoint.
...
def _extract_state_dict(ckpt_obj):
    if isinstance(ckpt_obj, dict):
        for key in ["model", "state_dict", "module", "teacher", "student", "network"]:
...
def _clean_state_dict_keys(state_dict):
    ...
    for k, v in state_dict.items():
        ...
        # Drop old classifier heads so the new 6-class head can be used.
        if new_k.startswith("head.") or new_k.startswith("fc.") or new_k.startswith("classifier."):
            continue
        new_sd[new_k] = v
    return new_sd


def load_panderm_checkpoint(model: nn.Module, checkpoint_path: str | Path, device: str = "cpu"):
    checkpoint_path = Path(checkpoint_path)
    print(f"Loading PanDerm checkpoint from: {checkpoint_path}")
...
def save_checkpoint(
    ...
    ckpt = {
        ...
        "backbone_name": "PanDerm_Base (ViT-B/16)",
    }
    ...
    for epoch in range(1, epochs + 1):
        ...
        if save_every_epoch:
            ...
            save_checkpoint(ep_path, model, optimizer, scheduler, epoch, val_metrics["balanced_accuracy"], stage_name,
                            class_to_idx, idx_to_class, image_size, pretrained_checkpoint)
...
def main() -> None:
    ...
    print("backbone=PanDerm_Base (ViT-B/16)")
    ...
    # PanDerm README uses imagenet_default_mean_and_std, so we stay with ImageNet normalization.
    ...
    print("=" * 80)
    print("STAGE 1: HAM with PanDerm Base")
    print("=" * 80)
    base_model = build_panderm_base_model(num_classes=len(PRIMARY_CLASSES)).to(device)
    base_model = load_panderm_checkpoint(base_model, args.pretrained_checkpoint, device=device)
    ...
    print("=" * 80)
    print("STAGE 2: BCN fine-tuning from PanDerm Base")
    print("=" * 80)
    ft_model = build_panderm_base_model(num_classes=len(PRIMARY_CLASSES)).to(device)
    ...
    ft_model, ft_out = fit_stage(
        ...
        stage_name="panderm_base_bcn",
        checkpoint_path=ckpt_dir / "panderm_base_bcn_ft_best.pt",
        metrics_json_path=metrics_dir / "panderm_base_bcn_val_metrics.json",
        device=device,
        class_names=PRIMARY_CLASSES,
        class_to_idx=class_to_idx,
        idx_to_class=idx_to_class,
    ...
    print("=" * 80)
    best_ft_model = build_panderm_base_model(num_classes=len(PRIMARY_CLASSES)).to(device)
    load_checkpoint_into_model(ckpt_dir / "panderm_base_bcn_ft_best.pt", best_ft_model, map_location=device)

    test_criterion = build_loss(bcn_weights, device=device, label_smoothing=0.0)
    test_metrics = evaluate(best_ft_model, test_bcn_loader, test_criterion, device, 0, "panderm_base_test_bcn", PRIMARY_CLASSES)
    save_json(test_metrics, metrics_dir / "panderm_base_test_bcn_metrics.json")

    test_cm = np.array(test_metrics["confusion_matrix"])
    plot_confusion_matrix(test_cm, PRIMARY_CLASSES, "PanDerm Base BCN Test Confusion Matrix", fig_dir / "panderm_base_test_bcn_confusion_matrix.png")

    test_pred_df = predict_with_metadata(best_ft_model, test_bcn_loader, device, idx_to_class)
    test_pred_df.to_csv(pred_dir / "panderm_base_test_bcn_predictions.csv", index=False)
    ...
    confusions.to_csv(metrics_dir / "panderm_base_test_bcn_top_confusions.csv", index=False)

    summary = {
        "backbone": "PanDerm_Base (ViT-B/16)",
        "pretrained_checkpoint": str(Path(args.pretrained_checkpoint).resolve()),
        "image_size": args.image_size,
        "device": device,
    ...
    save_json(summary, metrics_dir / "panderm_base_experiment_summary.json")
    print("=" * 80)
    print("DONE")
    print("=" * 80)
