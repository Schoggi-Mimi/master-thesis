...
echo "Launching CLS pooling lambda sweep"

for HA_LAMBDA in "${HA_LAMBDA_VALUES[@]}"; do

    # Format: sweep_ha__cls__l{lambda}__ep{epochs}__lr{lr}
    # Double underscore separates fields. Lambda uses 'p' for decimal point.
    LAMBDA_TAG=$(echo "$HA_LAMBDA" | sed 's/\./p/')
    RUN_NAME="sweep_ha__cls__l${LAMBDA_TAG}__ep${EPOCHS}__lr1e5"
...
echo "All CLS pooling jobs submitted."