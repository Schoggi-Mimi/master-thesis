...
INIT_CHECKPOINT=../external/checkpoints5/checkpoint-best-cls-ha5.pth
...
echo "Launching DAL sweep — CLS model — HA lambda fixed at ${HA_LAMBDA}"

for DAL_LAMBDA in "${DAL_LAMBDA_VALUES[@]}"; do

    LAMBDA_TAG=$(echo "$DAL_LAMBDA" | sed 's/\./p/')
    RUN_NAME="sweep_dal__cls__hal5__dall${LAMBDA_TAG}__ep${EPOCHS}__lr1e5"
...
echo "All CLS pooling DAL sweep jobs submitted."